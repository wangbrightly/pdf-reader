...
## 排版细节：标题分级/代码块/上标下标（2026-09-06/07，对比研究开源阅读器 mj_pdf 借鉴）

跟用户对比研究 mj_pdf（PDFium 内核的开源安卓 PDF 阅读器，GPLv3）之后借鉴的三个排版细节，用户明确选定范围只做这三个，首字下沉和 RTL 排版不做。三个功能改动都在抽取层（`classifyHeadings`/`linesToParagraphs`/`absorbSuperscriptSubscriptRuns`）+ 数据模型（`Line`/`Paragraph`/`DisplayBlock.Text`）+ 渲染层（`MainActivity.createParagraphTextView`），完整设计和 TDD 细节见 NOTES #67/#68/#69，这里只记结论和后续要注意的点。
