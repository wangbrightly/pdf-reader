...
- 版式复杂到没法可靠拆成"文字+独立图片"的页面（多图重叠拼贴、多栏混排、图标标签列……）会整页转成图片显示，保真优先于强行重排
...
- **代码量**：16880 行（Kotlin 主代码 9035 行、Kotlin 测试代码 7019 行、XML 826 行、Gradle 脚本 139 行），另有 2863 行 NOTES.md 踩坑记录。
- **提交次数**：140 次，标准 TDD 节奏（`test: ...(red)` 配对 `feat: ...(green)`，穿插真机 `fix` 和 `docs`）。累计新增 25448 行、删除/重写 2886 行——删除量不小，说明过程中有真实返工（比如按需加载架构分四步逐步替换旧实现、JBIG2 图片从"占位图"推翻重做成自己手写解码器）。
- **开发时长**：约 37.6 小时，跨 2026-08-17 至 2026-09-03 共 14 个自然日的持续投入，不是一天冲完的（按提交时间戳估算：相邻提交间隔 ≤90 分钟计入连续工作时间，超过按休息处理，此为估算方法，非精确计时）。
- **对比人工独立开发**：本项目自己实现了一套 PDF 内容抽取与重排引擎——解析 content stream 和坐标变换矩阵来修正图片朝向、按视觉顺序重排文字、检测表格网格、绕过 CMYK/异常位深导致的花屏、手写 JBIG2 算术解码器还原扫描图片、接入 JPX/JPEG2000 native 解码库，属于算法/领域相关工程，行业经验里人均日产出通常低于普通业务代码，估算约 150～250 行/日"测试通过、真机验证过"的代码。按净新增 22562 行估算，传统手写方式约需 **90～150 个工作日**（18～30 周）；AI 辅助后实际耗时约为其 1/19～1/32。此项为估算，非精确测量。
