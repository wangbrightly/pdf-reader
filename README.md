...
- **代码量**：13779 行（Kotlin 主代码 7668 行、Kotlin 测试代码 5198 行、XML 796 行、Gradle 脚本 117 行），另有 2023 行 NOTES.md 踩坑记录。
- **提交次数**：129 次，标准 TDD 节奏（`test: ...(red)` 配对 `feat: ...(green)`，穿插真机 `fix` 和 `docs`）。累计新增 21117 行、删除/重写 2720 行——删除量不小，说明过程中有真实返工（比如按需加载架构分四步逐步替换旧实现、JBIG2 图片从"占位图"推翻重做成自己手写解码器）。
- **开发时长**：约 35 小时，跨 2026-08-17 至 2026-08-26 共 9 个自然日的持续投入，不是一天冲完的（按提交时间戳估算：相邻提交间隔 ≤90 分钟计入连续工作时间，超过按休息处理，此为估算方法，非精确计时）。
- **对比人工独立开发**：本项目自己实现了一套 PDF 内容抽取与重排引擎——解析 content stream 和坐标变换矩阵来修正图片朝向、按视觉顺序重排文字、检测表格网格、绕过 CMYK/异常位深导致的花屏、手写 JBIG2 算术解码器还原扫描图片，属于算法/领域相关工程，行业经验里人均日产出通常低于普通业务代码，估算约 150～250 行/日"测试通过、真机验证过"的代码。按净新增 18397 行估算，传统手写方式约需 **74～123 个工作日**（15～25 周）；AI 辅助后实际耗时约为其 1/17～1/28。此项为估算，非精确测量。
