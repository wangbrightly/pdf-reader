...
- 图片、表格按原页面位置浮动展示，支持双指缩放；CMYK/YCCK JPEG、JBIG2、JPX/JPEG2000 这几种常见扫描件/设计稿图片格式都自己实现或接入了解码器，不会显示成花屏或纯黑块
- 版式复杂到没法可靠拆成"文字+独立图片"的页面（多图重叠拼贴、多栏混排、图标标签列……）会整页转成图片显示，保真优先于强行重排；表格与正文混排这类原来只能整页二选一的场景，会先问一次轻量级版面检测模型（PP-DocLayout-M，Apache-2.0），能局部裁出表格区域就只转那一块，正文保留重排能力，模型没把握时原样退回现有几何规则，不会让结果变差
- 标题按字号自动分三级（H1/H2/H3）放大展示；连续多行等宽字体的代码块用等宽字体渲染；脚注一类的纯数字上标/下标转换成正确的 Unicode 上标/下标字符，不跟正文数字混在一起
- 识别并跳过打印/导出 PDF 常见的页眉页脚水印（日期、网址、页码计数、排版软件自动加的文件名+定稿时间+页码版心标记）
- 目录（大纲）跳转，支持定位到页内精确位置，不只是跳到整页开头
- 字号/行距/边距/段距可调（旋钮式滑杆，每项独立强调色），即时生效并持久化
- 阅读进度按文件内容哈希 + 页码记忆，换了文件不会读错进度
- 深色模式（跟随系统）、系统栏适配
- 沉浸阅读模式：点击屏幕中央呼出/收起顶部工具栏和右侧翻页手柄，读的时候不占屏幕
- 顶部常显当前文件名，长文件名自动跑马灯滚动
- 大文档（几千页）按页懒加载，不会因为一次性渲染全文而内存溢出
- 页面文字优先展示、当前可见页优先加载，减少等待感
...
- **版面检测（局部裁剪表格用）**：[PP-DocLayout-M](https://huggingface.co/PaddlePaddle/PP-DocLayout-M)（百度 PaddleOCR/PaddleX 生态，Apache-2.0）+ [onnxruntime-android](https://github.com/microsoft/onnxruntime)（MIT）。原本考虑过参照另一款阅读器用 YOLOv12x 级别的模型做完整 CV 版面识别，实测/调研后确认那条路线的模型许可证是 AGPL-3.0（与个人闭源 App 天然冲突）且模型体量差一个数量级，正式否决；后来把范围收紧到"只在现有几何规则容易误判整页栅格化的场景，用 Apache-2.0 的轻量模型做一次辅助裁剪"，不移植 OCR/独立页面分类/合成引擎这些本项目已有对应实现的部分。完整决策复核过程见 [V0.3-DECISION.md](V0.3-DECISION.md)。
...
v0.3.0 起 APK 从约 14MB 涨到约 111MB——版面检测模型（`pp-doclayout-m.onnx`，约 22MB）和 `onnxruntime-android` 运行时（约 26MB）都打进了正式包，见上面"版面检测"一节。
...
- **代码量**：18865 行（Kotlin 主代码 9793 行、Kotlin 单元测试 7853 行、Kotlin instrumentation 测试 246 行、XML 826 行、Gradle 脚本 147 行），另有 3273 行 NOTES.md 踩坑记录。
- **提交次数**：152 次，标准 TDD 节奏（`test: ...(red)` 配对 `feat: ...(green)`，穿插真机 `fix` 和 `docs`）。累计新增 28169 行、删除/重写 3163 行——删除量不小，说明过程中有真实返工（比如按需加载架构分四步逐步替换旧实现、JBIG2 图片从"占位图"推翻重做成自己手写解码器）。
- **开发时长**：约 41.0 小时，跨 2026-08-17 至 2026-09-07 共 16 个有提交的自然日的持续投入，不是一天冲完的（按提交时间戳估算：相邻提交间隔 ≤90 分钟计入连续工作时间，超过按休息处理，此为估算方法，非精确计时）。
- **对比人工独立开发**：本项目自己实现了一套 PDF 内容抽取与重排引擎——解析 content stream 和坐标变换矩阵来修正图片朝向、按视觉顺序重排文字、检测表格网格、绕过 CMYK/异常位深导致的花屏、手写 JBIG2 算术解码器还原扫描图片、接入 JPX/JPEG2000 native 解码库和 ONNX 版面检测模型，属于算法/领域相关工程，行业经验里人均日产出通常低于普通业务代码，估算约 150～250 行/日"测试通过、真机验证过"的代码。按净新增 25006 行估算，传统手写方式约需 **100～167 个工作日**（20～33 周）；AI 辅助后实际耗时约为其 1/20～1/33。此项为估算，非精确测量。
