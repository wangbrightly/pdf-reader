 ...
 * 不是热路径，这笔交换划算。
 *
 * ## 验证状态（如实记录，别当成"已经验证过跟通用区域解码器一样扎实"）
 *
 * [Jbig2GenericRegionDecoder] 上线前做过严格的交叉验证：构造多组合成测试数据，
 * 跟参考实现逐像素比对，全部一致。这次符号词典/文字区域涉及的算术整数解码
 * 状态机更复杂，同样的合成数据验证方式本来也该做一遍，但本机没有 JBIG2
 * 编码器（`jbig2enc` 因为网络问题装不上），没法凭空构造出合法的符号词典/
 * 文字区域测试数据——原计划改成导出真机上这本书的一小份真实数据当 fixture，
 * 这一步在等设备解锁时被用户的下一个问题（CMYK JPEG）打断，**这份代码目前
 * 只做到"编译通过、没有破坏现有测试"，还没有做过任何形式的解码正确性验证
 * （既没有跟参考实现比对，也没有真机装机确认解出来的图像内容对不对）**。
 * 写完之后的代码审查阶段确实抓出并修了两处会导致解码结果整体错误的 bug
 * （符号词典的"导出符号数/新增符号数"两个字段顺序读反、`symbolCodeLength`
 * 计算逻辑冗余可能引入偏差），但代码审查能发现的是"看得出来的逻辑错误"，
 * 不能替代"跟一个可信实现比对真实解码结果"这种验证——这类状态机式的算法，
 * 完全可能还有代码审查看不出来、只有真的跑一遍数据才会暴露的问题（[Jbig2
 * GenericRegionDecoder] 的经验就是如此：那次的滑动窗口写法本身抄得很仔细，
 * 依然是靠交叉验证测试而不是人工审查建立起最终的信心）。继续这个功能时，
 * 优先级最高的下一步是把交叉验证这一课补上，不是想当然地假设"能编译、逻辑
 * 看着对"就等于"解码结果是对的"。
 */
